...
- (instancetype)initWithImageViewSize:(CGSize)imageViewSize
{
    self = [super init];
    if (self) {
        _imageViewFrame = CGRectMake(0, 0, imageViewSize.width, imageViewSize.height);
        [self initialize];
    }
    return self;
}
...
#pragma mark ---这个地方为了不去更换图片  所以直接在这里改变IMageView的尺寸
//        imageViewFrame = CGRectMake(0.f, 0.f, 24, 24);
        imageViewFrame = CGRectEqualToRect(self.imageViewFrame, CGRectZero) ? CGRectMake(0, 0, 24, 24):self.imageViewFrame;
