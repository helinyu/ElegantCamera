...
@property (nonatomic, copy  ) VoidBlock wk_dismissBlock;
...
- (void)setParentView:(UIView *)parentView
{
    ...
    UIView *markingView = nil;
    //寻找遮罩层
    for (UIView *subview in _parentView.subviews) {
        if (subview.tag == NightViewTag) {
            markingView = subview;
            break;
        }
    }
    if (markingView) {
        [_parentView  insertSubview:self belowSubview:markingView];
    }else{
        [_parentView addSubview:self];
    }
...
- (BOOL)isEqual:(id)object
{
    ...
    NSString * selfCN = [self className];
    NSString * objCN = [object className];
