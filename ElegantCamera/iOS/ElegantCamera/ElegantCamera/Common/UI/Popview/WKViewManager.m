            ...
            if ([obj isKindOfClass:NSClassFromString(cn)]) {
                ...
                else
                {
                    for (NSString * vn in viewNames) {
                        if ([vn isEqualToString:[view className]]) {
                            [vm.blackListView addObject:view];
                            return WKViewEnterQueueStateFailed_BlackListCheckFailed;
                        }
                    }
...
+(NSArray<WKBaseView *> *)getAllOperationViews
{
    WKViewManager * vm = [WKViewManager sharedManager];
    return [vm getAllOperationViews];
}
...
-(NSArray<WKBaseView *> *)getAllOperationViews
{
    ...
    for (NSOperationQueue * tmpQ in self.queueList) {
        for (WKViewOperation * op in [tmpQ operations]) {
            [views addSafeObject:op.view];
        }
    }
    return views;
}
...
{
    NSString * key = [NSString stringWithFormat:@"%@+%p",obj.className,obj];
...
+ (void)showWaitViewWithVC:(UIViewController *)vc
{
    ...
    if ([vc isKindOfClass:NSClassFromString([self blackListHomeVCClassName])])
    {
        [vm.blackListView enumerateObjectsUsingBlock:^(WKBaseView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [obj showInView:[AppDelegate getVisableView] isShow:YES];
        }];
        [vm.blackListView removeAllObjects];
    }
    //处理白名单
    NSString * vcClassName = [vc className];
    NSMutableArray * didShowViews = [NSMutableArray array];
    for (WKBaseView * baseView in vm.whiteListView) {
        NSArray * values = [[self whiteList] objectForKey:[baseView className]];//vc类名
...
+ (NSDictionary *)blackList
{
    ...
    dispatch_once(&onceToken, ^{
        blackList = @{
                      @"GuidePageViewController":@[WK_AllViewNickName],
                      @"ChooseSexViewController":@[WK_AllViewNickName],
                      @"WKAdvertisingVC":@[@"AppLaunchAdView"],
                      @"ComicReadViewController":@[@"AppLaunchAdView"],
                      @"XNComicReadFloatViewController":@[@"KMUpdatePopView"],
                      };
    });
    return blackList;
}
...
+ (NSString *)blackListHomeVCClassName
{
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        homeVCClassName = @"XNHomePageViewController";
    });
...
+ (NSDictionary *)whiteList
{
    ...
    dispatch_once(&onceToken, ^{
        whiteList = @{
//                      @"KMPopAlertView":@[@"NewHomeViewController"];//用法示例
