...
@property (nonatomic, assign)CGFloat humRadius;
...
static CGFloat const WSProgressHUDHeightToTxt = 60.0f;

static CGFloat const WSProgressHUDRadius = 7.0f;
...
{
    [self showLoading:WSProgressHUDLoadingTypeNormal];
}


+ (void)showLoading:(WSProgressHUDLoadingType)loadingType
{
    [self showLoading:loadingType maskType:WSProgressHUDMaskTypeDefault];
}

+ (void)showLoading:(WSProgressHUDLoadingType)loadingType maskType:(WSProgressHUDMaskType)maskType
{
    [[self shareInstance] addOverlayViewToWindow];
    [[self shareInstance] showWithString:nil maskType:maskType maskWithout:WSProgressHUDMaskWithoutDefault loadingType:loadingType];
}
...
+ (void)showMessage:(NSString *)message {
    [self showMessage:message alignment:WSProgressHUDAlignmentCenter];
}

+ (void)showBottomMessage:(NSString *)message
{
    [self showMessage:message alignment:WSProgressHUDAlignmentBottom];
}

+ (void)showMessage:(NSString *)message alignment:(WSProgressHUDAlignment)alignment
{
    [self showImage:nil status:message maskType:WSProgressHUDMaskTypeDefault maskWithout:WSProgressHUDMaskWithoutDefault alignment:alignment];
}
...
+ (void)showForceErrorMessage:(NSString *)string {
   [[self shareInstance] addOverlayViewToWindow];
   [[self shareInstance] showImage:WSProgressHUDErrorDefaultImage() status:string maskType:WSProgressHUDMaskTypeDefault maskWithout:WSProgressHUDMaskWithoutDefault alignment:WSProgressHUDAlignmentCenter];
}
...
{
    [self showImage:image status:title maskType:maskType maskWithout:withoutType alignment:WSProgressHUDAlignmentCenter];
}
...
{
    if (![self checkCanShow]) { //解冲pop冲突
        return;
    }
    [[self shareInstance] addOverlayViewToWindow];
    [[self shareInstance] showImage:image status:title maskType:maskType maskWithout:withoutType alignment:alignment];
}
...
+ (BOOL)checkCanShow
{
    NSArray<UIView *> *views = [WKViewManager getAllOperationViews];
    for (UIView *view in views) {
        if ([view isKindOfClass:[WKPopBaseView class]]) {
            if (!((WKPopBaseView *)view).isViewWillDismiss) {
                return NO;
            }
        }
    }
...
- (void)showWithString: (NSString *)string maskType: (WSProgressHUDMaskType)maskType maskWithout: (WSProgressHUDMaskWithoutType)withoutType
{
    ...
    objc_setAssociatedObject(self, @selector(alignment), @(WSProgressHUDAlignmentCenter), OBJC_ASSOCIATION_ASSIGN);
    objc_setAssociatedObject(self, @selector(loadingType), @(WSProgressHUDLoadingTypeNormal), OBJC_ASSOCIATION_ASSIGN);
...
- (void)showWithString: (NSString *)string maskType: (WSProgressHUDMaskType)maskType maskWithout: (WSProgressHUDMaskWithoutType)withoutType loadingType:(WSProgressHUDLoadingType)loadingType
{
    ...
    objc_setAssociatedObject(self, @selector(withoutType), @(withoutType), OBJC_ASSOCIATION_ASSIGN);
    objc_setAssociatedObject(self, @selector(alignment), @(WSProgressHUDAlignmentCenter), OBJC_ASSOCIATION_ASSIGN);
    objc_setAssociatedObject(self, @selector(loadingType), @(loadingType), OBJC_ASSOCIATION_ASSIGN);
...
- (void)showImage:(UIImage *)image status:(NSString *)title maskType: (WSProgressHUDMaskType)maskType maskWithout: (WSProgressHUDMaskWithoutType)withoutType alignment:(WSProgressHUDAlignment)alignment
{
    ...
    objc_setAssociatedObject(self, @selector(withoutType), @(withoutType), OBJC_ASSOCIATION_ASSIGN);
    objc_setAssociatedObject(self, @selector(alignment), @(alignment), OBJC_ASSOCIATION_ASSIGN);
...
- (void)dismiss
{
    ...
    objc_setAssociatedObject(self, @selector(hudAddingWindow), @(0), OBJC_ASSOCIATION_ASSIGN);
...
- (void)addOverlayViewToWindow
{
    ...
    objc_setAssociatedObject(self, @selector(hudAddingWindow), @(1), OBJC_ASSOCIATION_ASSIGN);
...
- (void)updateSubviewsPosition
{
    ...
    [self updateHumLabelView];
    
    CGSize hudSize = [self hudSizeWithString:string];
    CGRect hudBounds = CGRectMake(0, 0, hudSize.width, hudSize.height);
    if (self.hudType == WSProgressHUDTypeStatus) {
        hudBounds.size = CGSizeMake(hudBounds.size.width += 16, hudBounds.size.height += 16);
    }
    else if(self.hudType == WSProgressHUDTypeImage && self.imageView.image)
    {
        hudBounds.size = CGSizeMake(hudBounds.size.width += 20, hudBounds.size.height += 24);
    }
    else
    {
        hudBounds.size = CGSizeMake(hudBounds.size.width += 12, hudBounds.size.height += 20);
    }
...
-(void)updateHumLabelView
{
    self.humRadius = WSProgressHUDRadius;
    switch (self.hudType) {
        case WSProgressHUDTypeStatus: {
            self.labelView.font = HJTFont(14);
            self.labelView.textColor = kRGB_33;
        } break;
        ...
        case WSProgressHUDTypeImage: {
            ...
            if (self.imageView.image) {
                self.labelView.font = HJTFont(16);
                self.labelView.textColor = kRGB_33;

            } else {
                self.labelView.font = HJTFont(15);
                self.labelView.textColor = kRGB_33;
            }
    ...
    self.hudView.layer.cornerRadius = self.humRadius;
}
...
- (void)updatePositionWithString: (NSString *)string hudBounds: (CGRect)bounds
{
    ...
    //
    if (self.alignment == WSProgressHUDAlignmentBottom) {
        centerY = self.bounds.size.height - (TAB_BAR_HEIGHT + 40);
    }
    else if(self.alignment == WSProgressHUDAlignmentTop)
    {
        centerY = NAVIGATION_STATUS_HEIGHT + 60;
    }
    ...
    self.hudView.backgroundColor = kRGB_FF;
...
//                    }
                    self.indicatorView.activityIndicatorViewStyle = UIActivityIndicatorViewStyleGray; //设置为白色
                }
                self.labelView.frame = WSProgressHUDStringRect;
                self.labelView.center = CGPointMake(hudCenterX, WSProgressHUDHeightToTxt + WSProgressHUDStringRect.size.height/2);
                self.spinnerView.center = self.indefiniteAnimationView.center = self.indicatorView.center = CGPointMake(hudCenterX, 32);
                ...
                if (self.loadingType == WSProgressHUDLoadingTypeNormal) {
                    self.hudView.backgroundColor = kRGB_FF;
                    self.hudView.layer.borderWidth = 1.0f/[UIScreen mainScreen].scale;
                }
                else
                {
                    self.hudView.backgroundColor = kClearColor;
                    self.hudView.layer.borderWidth = 0;
                }
        ...
        case WSProgressHUDTypeImage: {
            ...
            if (self.imageView.image) {
                ...
                [self startIndicatorAnimation:NO];
                self.labelView.center = CGPointMake(hudCenterX, WSProgressHUDHeightToTxt + WSProgressHUDStringRect.size.height/2 + 5);
                self.labelView.textAlignment = NSTextAlignmentCenter;
                self.imageView.center = CGPointMake(hudCenterX, 40);
...
- (CGSize)hudSizeWithString: (NSString *)string
{
    ...
    WSProgressHUDDefaultHeight = 72.0f;
    WSProgressHUDDefaultWidth = 72.0f;
    ...
    switch (self.hudType) {
        case WSProgressHUDTypeStatus: {
            ...
            if (string.length) {
                ...
                WSProgressHUDDefaultWidth = MAX(WSProgressHUDDefaultWidth, 100);
                WSProgressHUDDefaultHeight = stringHeight + WSProgressHUDHeightToTxt + 2;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self setProgressHUDFont:HJTFont(14)];
...
- (UIView *)hudView
{
    if (!_hudView) {
        ...
        _hudView.backgroundColor = kRGB_FF;
        
        _hudView.layer.cornerRadius = WSProgressHUDRadius;
        _hudView.layer.borderWidth = 1.0f/[UIScreen mainScreen].scale;
        _hudView.layer.borderColor = kRGB_EF.CGColor;
        _hudView.layer.shadowColor = kRGB_00.CGColor;
        _hudView.layer.shadowOffset = CGSizeMake(0, 0);
        _hudView.layer.shadowOpacity = 0.12f;
...
- (UILabel *)labelView
{
    if (!_labelView) {
        ...
        if ([UIFont respondsToSelector:@selector(preferredFontForTextStyle:)]) {
            ...
            _labelView.font = HJTFont(font.pointSize);
        } else {
            _labelView.font = HJTFont(14);
        }
...
- (UILabel *)shimmeringLabel {
    if (!_shimmeringLabel) {
        ...
        if ([UIFont respondsToSelector:@selector(preferredFontForTextStyle:)]) {
            UIFont *font = [UIFont preferredFontForTextStyle:UIFontTextStyleBody];
            _shimmeringLabel.font = HJTFont(font.pointSize); //[UIFont preferredFontForTextStyle:UIFontTextStyleBody];
        } else {
            _shimmeringLabel.font = HJTFont(15);
        }
...
- (WSProgressHUDAlignment)alignment
{
    return [objc_getAssociatedObject(self, _cmd) integerValue];
}

- (WSProgressHUDLoadingType)loadingType
{
    return [objc_getAssociatedObject(self, _cmd) integerValue];
}
